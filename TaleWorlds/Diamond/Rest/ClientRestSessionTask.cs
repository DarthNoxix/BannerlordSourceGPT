...
internal class ClientRestSessionTask
{
	...
	private void ExamineResult()
	{
		if (!Request.Successful)
		{
			...
			else if (Request.Exception != null && Request.Exception is IOException)
			{
				TaleWorlds.Library.Debug.Print(string.Concat("Http Post Request with message(", RestRequestMessage, ")  failed. Retrying: (", Request.Exception?.GetType(), ") ", Request.Exception));
				flag = true;
			}
			else if (Request.Exception != null && Request.Exception is SocketException)
			{
				TaleWorlds.Library.Debug.Print(string.Concat("Http Post Request with message(", RestRequestMessage, ")  failed. Retrying: (", Request.Exception?.GetType(), ") ", Request.Exception));
				flag = true;
			}
			else if (Request.Exception != null && Request.Exception is HttpRequestException)
			{
				TaleWorlds.Library.Debug.Print(string.Concat("Http Post Request with message(", RestRequestMessage, ")  failed. Retrying: (", Request.Exception?.GetType(), ") ", Request.Exception));
				flag = true;
			}
